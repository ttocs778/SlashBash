...
{
    class LogJumpHandler
    {
        private const string LogCoreScriptName = nameof(LogCore);
        ...
        private static bool OnOpenAssetSoundLog(int instanceID)
        {
            ...
            if (!asset.Contains($"/{LogCoreScriptName}.cs")) //LogCore是笔者封装Debug.Log的MonoScripts文件  这里进行筛选
            {
                return false;
            }
            ...
            var activeText =
                m_ConsoleWindow?.GetField("m_ActiveText",
                    BindingFlags.Instance | BindingFlags.NonPublic); //m_ActiveText包含了当前Log的全部信息
            var consoleWindowFiledInfo =
                m_ConsoleWindow?.GetField("ms_ConsoleWindow",
                    BindingFlags.Static | BindingFlags.NonPublic); //ms_ConsoleWindow 是ConsoleWindow的对象字段
