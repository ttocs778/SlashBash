...
namespace YogiGameCore.Utils.MonoExtent.Editor
{
    [CustomEditor(typeof(MonoBehaviour), true)]
    [CanEditMultipleObjects]
    public class CustomAttributeEditor : UnityEditor.Editor
    {
        ...
        void DrawButtonForMethod(object[] invokationTargets, MethodInfo methodInfo, EditorButtonState state)
        {
            ...
            var methodDisplayName = MethodDisplayName(methodInfo,out var color);
            var origin = GUI.color;
            GUI.color = color;
            bool clicked = GUILayout.Button(methodDisplayName, GUILayout.ExpandWidth(true));
            GUI.color = origin;
        ...
        static object DrawVector4Parameter(ParameterInfo parameterInfo, object val)
        {
        ...
        string MethodDisplayName(MethodInfo method, out Color color)
        {
            var btnAttribute = method.GetCustomAttribute<ButtonAttribute>();
            string btnDisplay;
            color = btnAttribute.DisplayColor;
            if (string.IsNullOrEmpty(btnAttribute.Text))
            {
                btnDisplay = method.Name;
            }
            else
            {
                btnDisplay = btnAttribute.Text;
            }

            StringBuilder sb = new StringBuilder();
            sb.Append(btnDisplay + "(");
